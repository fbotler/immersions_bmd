...
and then to show that every graph whose complement admits such a \(3\)-coloring
contains an immersion of \(K_{n/2}\).
...
For \(X,Y \subseteq V(G)\), we use \(N_X(Y)\) to denote the set of neighbors of \(Y\) in \(X\), 
and, if \(X \cap Y = \emptyset\), then we denote by \(G[X,Y]\) the bipartite subgraph of \(G\) 
with vertex set \(X \cup Y\) and all edges of \(G\) between \(X\) and \(Y\). 

\begin{theorem}\label{thm:graphs_with_special_clique_coloring}
	Let \(G\) be a graph with \(\alpha(G) = 2\)
        that admits a \(3\)-clique coloring \(\{D_1,D_2,D_3\}\) such that
...
\begin{proof}
	...
	If \({|N_{D_1}(C)| < |C|}\), 
	then the set \({(D_1\setminus N_{D_1}(C))\cup C}\) is independent and is larger than \(D_1\),
	a contradiction to the maximality of \(D_1\).
	Hence, \(|N_{D_1}(C)| \geq |C|\) for every subset \(C\) of \(D_i\).
	So, by Hall's Theorem, there is a matching \(M_i\) in \(\Gcompl[D_i,D_1]\) that covers \(D_i\).
...
%	We say that \(r_u\) is the \emph{representative} of \(u\).
	Note that \(r_u \notin N(u)\),
	and hence, because \(\alpha(G) = 2\), \(r_u\) is adjacent to every vertex in \(V(G)\setminus N[u]\).
...
%	
	Note that if \(uv\notin E'\) we have \(r_u\in N(v)\) and \(r_v\in N(u)\),
	so, because \(D_1\) is a clique, \(r_ur_v\in E(G)\).
	...
	Analogously, we deduce that \(vr_u \neq v'r_{u'}\).
...
\begin{figure}[ht]
    \centering
        ...
        \end{tikzpicture}
    \caption{Situation in the proof of Theorem~\ref{thm:graphs_with_special_clique_coloring}.}
...
\begin{proof}[Proof of Theorem~\ref{thm:main}]
	Let \(G\) be a graph with \(n\) vertices for which \(\Delta(G) < 19n/29 - 1\) 
        and \(\alpha(G) \leq 2\).
...
%	
	...
	If \(|I_1| \geq n/2\), then \(G[I_1]\) is the desired immersion.
	Thus, assume \(|I_1| < n/2\).
	By Lemma~\ref{lemma:andrasfai_blowup_coloring},
	there exists a \(3\)-coloring \(\{I_1,I_2,I_3\}\) of \(\Gcompl\)
