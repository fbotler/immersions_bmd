...
then \(G\) is \(3\)-colorable.
%
Chen, Jin, and Koh~\cite{chen1997triangle} strenghtened Jin's result
...
\begin{proposition}
	...
	Then \(\Gamma_d\) admits a \(3\)-coloring \(\{D_1,D_2,D_3\}\)
	such that \(\overline{G}[D_2\cup D_3]\) has no induced \(C_4\).
\end{proposition}
...
\begin{proof}
	...
	As observed above, \(D_2\) and \(D_3\) are indepedent sets of \(\Gamma_d\).
	By the definition of \(E_d\), if \(u,u'\in D_2\),
	then either \(N_{D_3}(u) \subseteq N_{D_3}(u')\) or \(N_{D_3}(u') \subseteq N_{D_3}(u)\).
	Now, we claim that \(\overline{G}[D_2\cup D_3]\) has no induced \(C_4\).
	This is equivalent to prove that \(G[D_2\cup D_3]\) has no induced matching with two edges.
	Suppose, for a contradiction that \(G[D_2\cup D_3]\) has an induced matching with two edges \(M\).
	...
	then we have \(v' \in N_{D_3}(u)\), and hence \(G[\{u,u',v,v'\}\) is not an induced matching.
\end{proof}
