...
\begin{theorem}
Let \(G\) be a graph with \(n\) vertices for which \(\alpha(G) \leq 2\) and \(\Delta(G) \leq 19n/29\).
...
then \(G\) is \(3\)-colorable.
Chen, Jin, and Koh~\cite{chen1997triangle} showed the structure of this graph,
proving that if \(\delta(G) > 10n/29\),
then \(G\) is homomorphic to \(\Gamma_d\) for some \(d\in\mathbb{N}\),
where \(\Gamma_d\) is the graph \((V_d,E_d)\)
for which \(V_d = [3d-1]\) and \(E_d = \{xy : |x-y| \in [2d-1]\}\).
...
\begin{proposition}
	Let \(d\in\mathbb{N}\).
	Then \(\Gamma_d\) admits a \(3\)-coloring \(\{C_1,C_2,C_3\}\)
	such that (i) \(C_1\) is a maximum independent set of \(G\); and
	(ii) \(\overline{G}[C_2\cup C_3]\) has no induced \(C_4\)
\end{proposition}
...
\node[draw, circle, fill=black, inner sep=1pt, label=south:$v_3$] (c1) at (-18:1) {};
\node[draw, circle, fill=black, inner sep=1pt, label=north west:$v_2$] (c2) at (54:1) {};
\node[draw, circle, fill=black, inner sep=1pt, label=north:$v_1$] (c3) at (126:1) {};
\node[draw, circle, fill=black, inner sep=1pt, label=south:$v_5$] (c4) at (198:1) {};
\node[draw, circle, fill=black, inner sep=1pt, label=south:$v_4$] (c5) at (-90:1) {};
\node (C) at (0,0) {C};
...
\node[draw, circle, fill=black, inner sep=1pt] (y3) at (-2.5,0.8) {};
